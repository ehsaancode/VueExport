<template>
  <div ref="tablePaginationRPPRef" @click="handleClick" :style="containerStyle">
    <slot></slot>
  </div>
</template>
...
const tablePaginationRPPRef = ref(null);
...
onMounted(() => {
  ...
  if (tablePaginationRPPRef.value) {
    observer.observe(tablePaginationRPPRef.value);
  }
});

onUnmounted(() => {
  if (observer && tablePaginationRPPRef.value) {
    observer.unobserve(tablePaginationRPPRef.value);
  }
});
