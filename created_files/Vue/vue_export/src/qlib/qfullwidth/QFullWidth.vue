...
onMounted(() => {
  ...
  if (fullWidthRef.value) {
    observer.observe(fullWidthRef.value);
  }

  onUnmounted(() => {
    if (fullWidthRef.value) observer.unobserve(fullWidthRef.value);
  });
});

watch(isVisible, async (val) => {
  if (props.isAnimationP === "true" && val) {
    const reactRefMock = { current: fullWidthRef.value };
...
const combinedStyle = computed(() => {
  // Filter undefined/null props for generateStyle
  const styleProps = {
    width: props.width,
    height: props.height,
