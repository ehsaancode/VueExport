<template>
  <div ref="tableFilterRef" @click="handleClick" :style="containerStyle">
    <slot></slot>
  </div>
</template>
...
onMounted(() => {
  ...
  if (tableFilterRef.value) {
    observer.observe(tableFilterRef.value);
  }
});

onUnmounted(() => {
  if (observer && tableFilterRef.value) {
    observer.unobserve(tableFilterRef.value);
  }
});
