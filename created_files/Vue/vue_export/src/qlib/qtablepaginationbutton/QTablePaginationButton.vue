<template>
  <div ref="tablePaginationButtonRef" @click="handleClick" :style="containerStyle">
    <slot></slot>
  </div>
</template>
...
const tablePaginationButtonRef = ref(null);
...
onMounted(() => {
  ...
  if (tablePaginationButtonRef.value) {
    observer.observe(tablePaginationButtonRef.value);
  }
});

onUnmounted(() => {
  if (observer && tablePaginationButtonRef.value) {
    observer.unobserve(tablePaginationButtonRef.value);
  }
});
