<template>
  <div ref="tablePaginationInfoRef" @click="handleClick" :style="containerStyle">
    <slot></slot>
  </div>
</template>
...
const tablePaginationInfoRef = ref(null);
...
onMounted(() => {
  ...
  if (tablePaginationInfoRef.value) {
    observer.observe(tablePaginationInfoRef.value);
  }
});

onUnmounted(() => {
  if (observer && tablePaginationInfoRef.value) {
    observer.unobserve(tablePaginationInfoRef.value);
  }
});
