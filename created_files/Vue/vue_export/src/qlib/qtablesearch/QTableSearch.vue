<template>
  <div ref="tableSearchRef" @click="handleClick" :style="containerStyle">
    <slot></slot>
  </div>
</template>
...
onMounted(() => {
  ...
  if (tableSearchRef.value) {
    observer.observe(tableSearchRef.value);
  }
});

onUnmounted(() => {
  if (observer && tableSearchRef.value) {
    observer.unobserve(tableSearchRef.value);
  }
});
