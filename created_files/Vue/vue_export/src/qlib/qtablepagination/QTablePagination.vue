...
const totalItemsValue = computed(() =>
  globalSearch.value ||
  columnSearch.value === "Yes" ||
  filterSearch.value === "Yes"
    ? perPageCurrentData.value.length
    : totalItems.value
);
...
onMounted(() => {
  ...
  if (tablePaginationRef.value) {
    observer.observe(tablePaginationRef.value);
  }
});

onUnmounted(() => {
  if (observer && tablePaginationRef.value) {
    observer.unobserve(tablePaginationRef.value);
  }
});
