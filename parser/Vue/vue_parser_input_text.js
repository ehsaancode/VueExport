...
class ParserInputText {
  ...
  async parseInputText() {
   //console.log('this.parentId', this.parentId)
   //console.log('formInfo', JSON.stringify (await getFormConfigForItem.getFormConfigForItem(this.projectId, this.pageId, this.parentId)));
  ...
  async endFile(pageName, startingColumn) {
    await readWriteFile.writeToFile(
      pageName,
      "\n" + " ".repeat(startingColumn + 1) + "</QInputText>\n" + ""
    );
  }
}
