...
const defaultParallax = require("../Common/parse_default_parallax");
...
class ParserGroupParallax {
  ...
  async parseGroupParallax() {
    ...
    await commonUtilsReact.generateVueTemplate({
      componentName: "QHorizontalParallax",
      isSelfClosing: false,
  ...
  async endFile(pageName, startingColumn) {
    await readWriteFile.writeToFile(
      pageName,
      "\n" + " ".repeat(startingColumn + 1) + "</QHorizontalParallax>\n" + ""
    );
  }
}
