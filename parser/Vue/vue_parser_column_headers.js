...
class ParserColumnHeaders {
  ...
  async parseColumnHeaders() {
    const metadata = tableMetadata.getInstance();
    metadata.columnHeadersId = String(this.jsonObjects?.id ?? "");
      ...
      if (tableType === 'customDynamic') {
        if (index == 0) {
          await createPageDesign.pageDesign(
            ...
            this.isAbsoluteValue,
            this.parantType === "QHMenuItem" ? "QHMenuItem" : "QColumnHeaders",
  ...
  async endFile(pageName, startingColumn) {
    await readWriteFile.writeToFile(
      pageName,
      "\n" + " ".repeat(startingColumn + 1) + "</QColumnHeaders>\n" + ""
    );
  }
}
