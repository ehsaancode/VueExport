...
const defaultParseVideo = require("../Common/parse_default_video");
...
class ParserVideo {
  ...
  async parseVideo() {
    ...
    let defaultObject = (await defaultParseVideo.getDefaultObject()) ?? "";
  ...
  async parsePage(
    ...
    for (const index in jsonObjects) {
      ...
      await createPageDesign.pageDesign(
        ...
        this.isAbsoluteValue,
        this.parantType === "QHMenuItem" ? "QHMenuItem" : "QVideoNetwork"
      );
    }
  }
  ...
  async endFile(pageName, startingColumn) {
    await readWriteFile.writeToFile(
      pageName,
      "\n" + " ".repeat(startingColumn + 1) + "</QVideo>\n" + ""
    );
  }
}
