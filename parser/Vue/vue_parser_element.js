...
class ParserElement {
  ...
  async parseElement() {
    ...
    if (cmsPageId && cmsPageId > 0) {
      let elementName = await commonUtilsReact.getElementNameByPageId(
        cmsPageId
      );
      ...
      metadata.importFiles = {
        key: lastComponent,
        value: `import ${elementName} from \"../../../elements/${elementName}/${elementName}_index.vue\"\n`,
      };
      /*await readWriteFile.replaceWordInFile(
        this.fileName,
        "[import_files]",
        `import ${elementName} from \"../../elements/${elementName}/${elementName}_index\"\n[import_files]`
      );*/
      // inject actual variable name
      // await readWriteFile.writeToFile(this.fileName, `\n<${elementName}/>\n`);
      await readWriteFile.writeToFile(
        this.fileName,
        " ".repeat(this.startingColum + 4) + `<${elementName}/>\n` + ""
      );
    } else {
