...
const defaultParseAccordion = require("../Common/parse_default_accordion");
...
class ParserAccordion {
  ...
  async parseAccordion() {
    ...
    let defaultObject = await defaultParseAccordion.getDefaultObject();

    await commonUtilsReact.generateVueTemplate({
      componentName: "QAccordion",
      isSelfClosing: false,
  ...
  async endFile(pageName, startingColumn) {
    await readWriteFile.writeToFile(
      pageName,
      "\n" + " ".repeat(startingColumn + 1) + "</QAccordion>\n" + ""
    );
  }
}
