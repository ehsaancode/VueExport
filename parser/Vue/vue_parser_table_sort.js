...
const defaultParseTableSort = require("../Common/parse_default_table_sort");
const reactJsMapper = require("../../mapper/vuejs/vuejs_mapper");


class ParserTableSort {
  constructor(
    projectId = 0,
    pageId = 0,
    startingColum,
  ...
  async parsePage(
    ...
    for (const index in jsonObjects) {
      ...
      await createPageDesign.pageDesign(
        ...
        this.isAbsoluteValue,
        this.parantType === "QHMenuItem" ? "QHMenuItem" : "QTableSort"
      );
    }
  }
  ...
  async endFile(pageName, startingColumn) {
    await readWriteFile.writeToFile(
      pageName,
      "\n" + " ".repeat(startingColumn + 1) + "</QTableSort>\n" + ""
    );
  }
}
