...
const defaultParseDropDown = require("../Common/parse_default_drop_down");
...
class ParserDropdown {
  ...
  async parseDropdown() {
    ...
    let defaultObject = (await defaultParseDropDown.getDefaultObject()) ?? "";
  ...
  async endFile(pageName, startingColumn) {
    await readWriteFile.writeToFile(
      pageName,
      "\n" + " ".repeat(startingColumn + 1) + "</QDropDown>\n" + ""
    );
  }
}
