...
class AndroidParserMarquee{
   ...
   async parseMarquee(){
        ...
        let defaultJsonObject = await commonDefault.getDefaultObject();
        let frame = await commonUtilsAndroid.componentFrame(this.jsonObjects, defaultJsonObject, this.parentWidth, this.parentHeight, false, true);
        ...
        if(this.parentType == "QStack"){
         ...
         await readWriteFile.writeToFile(
            this.fileName,
            ' '.repeat(this.startingColum) + 'QMarquee(\n' +
            ' '.repeat(this.startingColum + 1) + `${frame} \n` +
            ' '.repeat(this.startingColum + 1) + `children = listOf { \n`
         ).then(async (content) => {
            if (content == "success") {
                let widthHeight = await commonUtils.getComponentWidthHeight(this.jsonObjects, defaultJsonObject, this.parentWidth, this.parentHeight, true);
                                                      
                //console.log("*********** success Marquee ************");
   ...
   async parsePage(startingColum, fileName, jsonObjects, parentWidth, parentHeight) {    
        for (const index in jsonObjects) {
            ...
            await createPageDesign.pageDesign(startingColum, fileName, jsonObj, parentWidth, parentHeight, "QMarquee");
        }
    }
