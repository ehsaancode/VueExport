...
class AndroidParserFlex {
     ...
     async parseFlex() {
          ...
          let layoutProperties = await commonUtilsAndroid.componentLayoutProperties(widgetStyle);
          // console.log(`Result border properties - ${JSON.stringify(widgetStyle?.layout)}`);

          const displayType = widgetStyle?.layout?.displayType;
          const componentType = (() => {
               switch (displayType) {
                    case 'Block':
                         return `QContainer`;
          ...
          //Widget properties
          if (displayType === 'Flex') {
               output += layoutProperties['flexDirection'] !== ''
                    ? ' '.repeat(this.startingColum + 4) + `flexDirection = "${layoutProperties['flexDirection']}",\n` : '';
               output += layoutProperties['alignItems'] !== ''
                    ? ' '.repeat(this.startingColum + 4) + `alignItems = "${layoutProperties['alignItems']}",\n` : '';
               output += layoutProperties['justifyContent'] !== ''
                    ? ' '.repeat(this.startingColum + 4) + `justifyContent = "${layoutProperties['justifyContent']}",\n` : '';
               
               output += layoutProperties['flexWrap'] !== ''
                    ? ' '.repeat(this.startingColum + 4) + `flexWrap = "${layoutProperties['flexWrap']}",\n` : '';
               output += layoutProperties['flexWrapTitle'] !== ''
                    ? ' '.repeat(this.startingColum + 4) + `flexWrapTitle = "${layoutProperties['flexWrapTitle']}",\n` : '';

               output += layoutProperties['vSpace'] !== ''
                    ? ' '.repeat(this.startingColum + 4) + `vSpace = "${layoutProperties['vSpace']}",\n` : '';
               output += layoutProperties['hSpace'] !== ''
                    ? ' '.repeat(this.startingColum + 4) + `hSpace = "${layoutProperties['hSpace']}",\n` : '';
          }

          //padding properties
          output += (calculatePadding['top'] && calculatePadding['top'] !== "0")
               ? ' '.repeat(this.startingColum + 4) + `paddingTop = "${calculatePadding['top']}",\n` : '';
          output += (calculatePadding['bottom'] && calculatePadding['bottom'] !== "0")
               ? ' '.repeat(this.startingColum + 4) + `paddingBottom = "${calculatePadding['bottom']}",\n` : '';
          output += (calculatePadding['right'] && calculatePadding['right'] !== "0")
               ? ' '.repeat(this.startingColum + 4) + `paddingEnd = "${calculatePadding['right']}",\n` : '';
          output += (calculatePadding['left'] && calculatePadding['left'] !== "0")
               ? ' '.repeat(this.startingColum + 4) + `paddingStart = "${calculatePadding['left']}",\n` : '';

          //margin properties
          output += (calculateMargin['top'] && calculateMargin['top'] !== "0")
               ? ' '.repeat(this.startingColum + 4) + `marginTop = "${calculateMargin['top']}",\n` : '';
          output += (calculateMargin['bottom'] && calculateMargin['bottom'] !== "0")
               ? ' '.repeat(this.startingColum + 4) + `marginBottom = "${calculateMargin['bottom']}",\n` : '';
          output += (calculateMargin['left'] && calculateMargin['left'] !== "0")
               ? ' '.repeat(this.startingColum + 4) + `marginStart = "${calculateMargin['left']}",\n` : '';
          output += (calculateMargin['right'] && calculateMargin['right'] !== "0")
          ...
          await readWriteFile.writeToFile(this.fileName,
               ' '.repeat(this.startingColum) + `{ parentWidth, parentHeight ->\n` +
               ' '.repeat(this.startingColum + 2) + `${componentType}( \n` +
               output +
               ' '.repeat(this.startingColum + 4) + `children = listOf ( \n`//{
          ).then(async (content) => {
               if (content == "success") {
     ...
     async parsePage(startingColum, fileName, jsonObjects, parentWidth, parentHeight) {
          for (const index in jsonObjects) {
               ...
               await createPageDesign.pageDesign(startingColum, fileName, jsonObj, parentWidth, parentHeight, "QFlex");
          }
     }
