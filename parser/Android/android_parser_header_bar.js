...
class AndroidParserHeaderBar {
     ...
     async parseHeaderBar(){
          // console.log("data : parseHeaderBar");
          ...
          let pageName = './created_files/Android/HeaderBarView.kt';
          ...
          await readWriteFile.writeToFile(this.fileName,
               ...
               "@Composable\n" +
               `fun ` + `HeaderBarView(){ \n\n` +
               ' '.repeat(this.startingColum) + 'QHeaderView( \n'+ 
                    ...
                    if (content == 'success') {
                         if (this.jsonObjects["isFixed"] === true) {
                              ...
                              await readWriteFile.writeToFile(pageName,
                                  ...
                                  "@Composable\n" +
                                  `fun ` + `HeaderBarView(){ \n\n` + //Row {
                                   ' '.repeat(this.startingColum + 1) + 'Row{\n'
                              ).then(async content =>{
                                   if (content == 'success') {
                                        let getDefaultObject = await commonDefault.getDefaultObject();
                                        let widthHeight = await commonUtils.getComponentWidthHeight(this.jsonObjects, getDefaultObject, this.parentWidth, this.parentHeight, true);
     ...
     async parsePage(startingColum, fileName, jsonObjects, parentWidth, parentHeight) { 
          for (const index in jsonObjects) {
               ...
               await createPageDesign.pageDesign(startingColum + 1, fileName, jsonObj, parentWidth, parentHeight, "QHeaderBar"); 
          }
     }
