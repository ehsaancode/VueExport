...
class ParserTablePaginationRPP {
  constructor(
    projectId = 0,
    pageId = 0,
    startingColum,
  ...
  async parseTablePaginationRPP() {
    ...
    await commonUtilsReact.generateJSX({
      componentName: "QTablePaginationRPP",
      isSelfClosing: false,
  ...
  async parsePage(
    ...
    for (const index in jsonObjects) {
      ...
      await createPageDesign.pageDesign(
        ...
        this.isAbsoluteValue,
        this.parantType === "QHMenuItem" ? "QHMenuItem" : "QTablePaginationRPP"
      );
    }
  }
  ...
  async endFile(pageName, startingColumn) {
    await readWriteFile.writeToFile(
      pageName,
      "\n" + " ".repeat(startingColumn + 1) + "</QTablePaginationRPP>\n" + ""
    );
  }
}
