...
class ParserDashedProgressbar {
  ...
  async parseDashedProgressbar() {
    ...
    await commonUtilsReact.generateJSX({
      componentName: "QDashedProgressbar",
      isSelfClosing: false,
  ...
  async parsePage(
    ...
    for (const index in jsonObjects) {
      ...
      await createPageDesign.pageDesign(
        ...
        this.isAbsoluteValue,
        this.parantType === "QHMenuItem" ? "QHMenuItem" : "QDashedProgressbar"
      );
    }
  }
  ...
  async endFile(pageName, startingColumn) {
    await readWriteFile.writeToFile(
      pageName,
      "\n" + " ".repeat(startingColumn + 1) + "</QDashedProgressbar>\n" + ""
    );
  }
}

module.exports = {
  // parseDashedProgressbar,
  ParserDashedProgressbar,
};
