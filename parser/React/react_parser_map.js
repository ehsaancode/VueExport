...
const defaultParseMap = require("../Common/parse_default_map");
...
class ParserMap {
  ...
  async parseMap() {
    ...
    let defaultObject = (await defaultParseMap.getDefaultObject()) ?? "";
  ...
  async parsePage(
    ...
    for (const index in jsonObjects) {
      ...
      await createPageDesign.pageDesign(
        ...
        this.isAbsoluteValue,
        this.parantType === "QHMenuItem" ? "QHMenuItem" : "QMap"
      );
    }
  }
