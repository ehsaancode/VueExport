...
const defaultParseMarquee = require("../Common/parse_default_marquee");
...
class ParserMarquee {
  ...
  async parseMarquee() {
    ...
    let defaultObject = (await defaultParseMarquee.getDefaultObject()) ?? "";
  ...
  async endFile(pageName, startingColumn) {
    await readWriteFile.writeToFile(
      pageName,
      "\n" + " ".repeat(startingColumn + 1) + "</QMarquee>\n" + ""
    );
  }
}
