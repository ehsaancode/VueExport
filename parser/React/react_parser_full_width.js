...
const defaultParseFullWidth = require("../Common/parse_default_full_width");
...
class ParseFullWidth {
  ...
  async parseFullWidth() {
    ...
    let defaultObject = (await defaultParseFullWidth.getDefaultObject()) ?? "";

    await commonUtilsReact.generateJSX({
      componentName: "QFullWidth",
      isSelfClosing: false,
  ...
  async parsePage(
    ...
    for (const index in jsonObjects) {
      ...
      await createPageDesign.pageDesign(
        ...
        this.isAbsoluteValue,
         this.parantType == "QHMenuItem" ? "QHMenuItem" : (this.parantType == "QFormItem" || this.parantType == "QForm") ? this.parantType : "QFullWidth",
        '',
  ...
  async endFile(pageName, startingColumn) {
    await readWriteFile.writeToFile(
      pageName,
      "\n" + " ".repeat(startingColumn + 1) + "</QFullWidth>\n" + ""
    );
  }
}
