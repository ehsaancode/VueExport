...
class ParserColumnHeader {
  ...
  async parseColumnHeader() {
    ...
    let width = await commonUtilsReact.componentFrameWidth(
      ...
      this.isAbsoluteValue == true ? false : true
       ...
       if (metadata.tableType == "customDynamic"  ) {
              await readWriteFile.writeToFile(
            this.fileName,
            "\n" +
              " ".repeat(  1) +
              `<QRepeat widgetId="${metadata.columnHeadersId}">\n` +
              ""
          );
        }
  ...
  async parsePage(
    ...
    for (const index in jsonObjects) {
      ...
      await createPageDesign.pageDesign(
        ...
        this.isAbsoluteValue,
        this.parantType === "QHMenuItem" ? "QHMenuItem" : "QColumnHeader"
      );
    }
  }
  ...
  async endFile(pageName, startingColumn) {
    await readWriteFile.writeToFile(
      pageName,
      "\n" + " ".repeat(startingColumn + 1) + "</QColumnHeader>\n" + ""
    );
  }
}
