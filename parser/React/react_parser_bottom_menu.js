...
const defaultParseBottomMenu = require("../Common/parse_default_bottom_menu");
...
class ParserBottomMenu {
  ...
  async parseBottomMenu() {
    ...
    let defaultObject = (await defaultParseBottomMenu.getDefaultObject()) ?? "";
  ...
  async parsePage(
    ...
    for (const index in jsonObjects) {
      ...
      await createPageDesign.pageDesign(
        ...
        this.isAbsoluteValue,
        this.parantType === "QHMenuItem" ? "QHMenuItem" : "QBottomMenu"
      );
    }
  }
  ...
  async endFile(pageName, startingColumn) {
    await readWriteFile.writeToFile(
      pageName,
      "\n" + " ".repeat(startingColumn + 1) + "</QBottomMenu>\n" + ""
    );
  }
}
