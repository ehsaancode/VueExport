...
const defaultParseFlex = require("../Common/parse_default_flex");
...
class ParserFlex {
  ...
  async parseFlex() {
    ...
    let defaultObject = (await defaultParseFlex.getDefaultObject()) ?? "";
  ...
  async parsePage(
    ...
    for (const index in jsonObjects) {
      ...
      await createPageDesign.pageDesign(
        ...
        this.isAbsoluteValue,
        this.parantType == "QHMenuItem" ? "QHMenuItem" : (this.parantType == "QFormItem" || this.parantType == "QForm") ? this.parantType : "QFlex",
        '',
