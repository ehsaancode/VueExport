...
const defaultParseTableSearch = require("../Common/parse_default_table_search");
const reactJsMapper = require("../../mapper/reactjs/reactjs_mapper");


class ParserTableSearch {
  constructor(
    projectId = 0,
    pageId = 0,
    startingColum,
  ...
  async parsePage(
    ...
    for (const index in jsonObjects) {
      ...
      await createPageDesign.pageDesign(
        ...
        this.isAbsoluteValue,
        this.parantType === "QHMenuItem" ? "QHMenuItem" : "QTableSearch"
      );
    }
  }
  ...
  async endFile(pageName, startingColumn) {
    await readWriteFile.writeToFile(
      pageName,
      "\n" + " ".repeat(startingColumn + 1) + "</QTableSearch>\n" + ""
    );
  }
}
