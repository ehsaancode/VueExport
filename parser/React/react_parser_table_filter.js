...
const defaultParseTableFilter = require("../Common/parse_default_table_filter");
const reactJsMapper = require("../../mapper/reactjs/reactjs_mapper");


class ParserTableFilter {
  constructor(
    projectId = 0,
    pageId = 0,
    startingColum,
  ...
  async parsePage(
    ...
    for (const index in jsonObjects) {
      ...
      await createPageDesign.pageDesign(
        ...
        this.isAbsoluteValue,
        this.parantType === "QHMenuItem" ? "QHMenuItem" : "QTableFilter"
      );
    }
  }
  ...
  async endFile(pageName, startingColumn) {
    await readWriteFile.writeToFile(
      pageName,
      "\n" + " ".repeat(startingColumn + 1) + "</QTableFilter>\n" + ""
    );
  }
}
