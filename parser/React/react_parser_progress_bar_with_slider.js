...
class ParserProgressbarWithSlider {
  constructor(
    projectId = 0,
    pageId = 0,
    startingColum,
  ...
  async parseProgressbarWithSlider() {
    ...
    await commonUtilsReact.generateJSX({
      componentName: "QProgressbarWithSlider",
      isSelfClosing: false,
  ...
  async endFile(pageName, startingColumn) {
    await readWriteFile.writeToFile(
      pageName,
      "\n" + " ".repeat(startingColumn + 1) + "</QProgressbarWithSlider>\n" + ""
    );
  }
}

module.exports = {
  // parseProgressbarWithSlider,
  ParserProgressbarWithSlider,
};
