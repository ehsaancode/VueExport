...
class ParserFloatingButton {
  ...
  async parseFloatingButton() {
    ...
    let defaultObject =
      (await defaultParseFloatingButton.getDefaultObject()) ?? "";
  ...
  async endFile(pageName, startingColumn) {
    await readWriteFile.writeToFile(
      pageName,
      "\n" + " ".repeat(startingColumn + 1) + "</QFloatingButton>\n" + ""
    );
  }
}
