...
const defaultParseSlider = require("../Common/parse_default_slider");
...
class ParserSlider {
  ...
  async parseSlider() {
    ...
    let defaultObject = (await defaultParseSlider.getDefaultObject()) ?? "";
  ...
  async endFile(pageName, startingColumn) {
    await readWriteFile.writeToFile(
      pageName,
      "\n" + " ".repeat(startingColumn + 1) + "</QSlider>\n" + ""
    );
  }
}
