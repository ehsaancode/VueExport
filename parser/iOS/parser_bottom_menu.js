...
class ParserBottomMenu {
     ...
     async parseBottomMenu() {
          if (this.jsonObjects["isFixed"] === true) {
               let pageName = './created_files/iOS/BottomMenuView.swift'; // + jsonObject['webdesign_Attrs']['wstyle']['page-name'] + '.swift';
     ...
     async parsePage(startingColum, fileName, jsonObjects, parentWidth, parentHeight, parantMainAlignment, parantCrossAlignment) {
          for (const index in jsonObjects) {
               ...
               await createPageDesign.pageDesign(startingColum + 1, fileName, jsonObj, parentWidth, parentHeight, parantMainAlignment, parantCrossAlignment, "QBottomMenu");
     ...
     async componentFrame() {
          let defaultObj = await commonDefault.getDefaultObject();
          let mainAlignment = (this.jsonObjects["mainAlignment"] ?? defaultObj["mainAlignment"]) ?? this.parantMainAlignment;
          let crossAlignment = (this.jsonObjects["crossAlignment"] ?? defaultObj["crossAlignment"]) ?? this.parantCrossAlignment;
          let widthHeight = await commonUtils.getComponentWidthHeight(this.jsonObjects, defaultObj, this.parentWidth, this.parentHeight, false, false, true);
          ...
          if(width > 0 || height > 0) {
               ...
               for (const index in this.jsonObjects['children']) {
                    var jsonObj = jsonObjects[index];
                    let widthHeight = await createPageDesign.componentFrame(jsonObj, this.parentWidth, this.parentHeight, mainAlignment, crossAlignment);
