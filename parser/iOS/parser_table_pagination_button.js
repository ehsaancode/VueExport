...
const commonDefault = require('../Common/parse_default_table_pagination_button');
const commonUtilsIos = require('./common_utilits_ios');
const parserAnimations = require('./parser_animations');

async function parseTablePaginationButton(startingColum, fileName, jsonObjects, parentWidth, parentHeight, parantMainAlignment, parantCrossAlignment, parantType) {
     ...
     await readWriteFile.writeToFile(fileName,
          ' '.repeat(startingColum + 1) + 'QTablePaginationButton(minNumber: 1, maxNumber: 5) \n'
     );
