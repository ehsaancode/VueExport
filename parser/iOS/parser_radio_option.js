...
const commonDefault = require('../Common/parse_default_radio_option');
const commonUtilsIos = require('./common_utilits_ios');
const commonUtils = require('../../utility/common_utils');

async function parseRadioOption(startingColum, fileName, jsonObjects, parentWidth, parentHeight, parantMainAlignment, parantCrossAlignment, parantType) {
     ...
     await readWriteFile.writeToFile(fileName,
          ...
          ' '.repeat(startingColum + 1) + 'QRadioOption(\n' +
          ' '.repeat(startingColum + 2) + `items: ${optionsObj}, \n` +
          ' '.repeat(startingColum + 2) + `selectedId: \"${options[0]}\" \n` + 
