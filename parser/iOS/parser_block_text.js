...
const parserSlugPage = require("./parser_slug_page");

async function parseBlockText(
  startingColum,
  fileName,
  jsonObjects,
  parentWidth,
  ...
  // console.log(`Text parantType: ${parantType}`);
  let defaultObj =
    (await commonDefault.getDefaultObject(jsonObjects["type"])) ?? "";
  ...
  let formattedText = await commonUtilsIos.formatText(text); //text.split(/(?<=\.)/).join("\n");
  let forgroundColor = jsonObjects["color"] ?? "";
  let fontSize =
    jsonObjects["fontSize"] ??
    (await commonDefault.getDefaultValue(jsonObjects["type"], "fontSize")) ??
    "12px";
  ...
  if (jsonOnClick != undefined) {
    ...
    if (action != undefined && action == "Navigate to") {
      let destination = jsonOnClick["destination"].trim();
