...
const commonDefault = require('../Common/parse_default_table_sort');
const commonUtilsIos = require('./common_utilits_ios');
const parserAnimations = require('./parser_animations');

async function parseTableSort(startingColum, fileName, jsonObjects, parentWidth, parentHeight, parantMainAlignment, parantCrossAlignment, parantType) {
     ...
     await readWriteFile.writeToFile(fileName,
          ' '.repeat(startingColum + 1) + 'QSortButton { isAscending in \n' +
          ' '.repeat(startingColum + 2) + `print("Sorting \\(isAscending ? "ascending" : "descending")") \n` +
          ' '.repeat(startingColum + 1) + `} \n` +
          ' '.repeat(startingColum + 1) + `.padding() \n` +
          ' '.repeat(startingColum + 1) + `.previewLayout(.sizeThatFits) \n`
     );
