...
const commonDefault = require('../Common/parse_default_table_pagination_Info');
const commonUtilsIos = require('./common_utilits_ios');
const parserAnimations = require('./parser_animations');

async function parseTablePaginationInfo(startingColum, fileName, jsonObjects, parentWidth, parentHeight, parantMainAlignment, parantCrossAlignment, parantType) {
     ...
     await readWriteFile.writeToFile(fileName,
          ' '.repeat(startingColum + 1) + 'QTablePaginationInfo(fromCount: 1, toCount: 10, totalNumberOfData: 10) \n' +
