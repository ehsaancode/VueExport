...
const commonDefault = require('../Common/parse_default_table_filter');
const commonUtilsIos = require('./common_utilits_ios');
const parserAnimations = require('./parser_animations');

async function parserTableFilter(startingColum, fileName, jsonObjects, parentWidth, parentHeight, parantMainAlignment, parantCrossAlignment, parantType) {
     ...
     await readWriteFile.writeToFile(fileName,
          ' '.repeat(startingColum + 1) + 'QFilterButton() \n' +
          ' '.repeat(startingColum + 2) + `.padding() \n` +
          ' '.repeat(startingColum + 2) + `.previewLayout(.sizeThatFits) \n`
     );
