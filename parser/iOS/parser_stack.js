...
class ParserStack {
     ...
     async parseStack() {    
          ...
          await readWriteFile.writeToFile(this.fileName,
               ' '.repeat(this.startingColum) + `ZStack (${alignment}) { \n`
          ).then(async content => {
               if (content == 'success') {
     ...
     async parsePage(startingColum, fileName, jsonObjects, parentWidth, parentHeight, parantMainAlignment, parantCrossAlignment) {
          for (const index in jsonObjects) {
               ...
               await createPageDesign.pageDesign(startingColum + 1, fileName, jsonObj, parentWidth, parentHeight, parantMainAlignment, parantCrossAlignment, "QStack");
          }
     }
