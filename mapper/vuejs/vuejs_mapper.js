...
const reactjsMapperOnclick = require("./vuejs_mapper_onclick");
const reactjsMapperOnload = require("./vuejs_mapper_onload");
...
async function startReactjsMapper(projectId, pageId, fileName, widgetID) {
  ...
  let subProjectPath = projectId;
  let jsonPath = `${vueProjectPath}/${subProjectPath}/jsons/action_${pageId}.json`;
  await delete require.cache[require.resolve(jsonPath)];
  ...
  if (onLoadObjects?.length > 0) {
    ...
    await apiCallMapper.apiCallMapperOnLoad(
      `${vueProjectPath}/${subProjectPath}/jsons`,
      fileName,
      onLoadObjects[0],
      widgetID,
      pageId
